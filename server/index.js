...
app.get('/api/data', async (req, res) => {
    ...
    const weatherData = {
        current: {
            time: moment(new Date((Number(current?.time()) + utcOffsetSeconds) * 1000)).format("YYYY-MM-DD HH:mm:ss"),
            temperature2m: current?.variables(0)?.value(),
            weather: weatherCodes[weatherCode],
        },
        hourly: {
            time: range(Number(hourly?.time()), Number(hourly?.timeEnd()), hourly?.interval()).map(
                (t) => moment(new Date((t + utcOffsetSeconds) * 1000)).format("YYYY-MM-DD HH:mm:ss")
            ),
